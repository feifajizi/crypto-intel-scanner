...
// 记录所有查到的链接（不只是有质押的），供 enrichment 使用
const allScannedLinks = new Map();
...
async function scanStaking(coins) {
  ...
  for (let i = 0; i < coins.length; i++) {
    ...
    // 记录链接（即使没有质押也记录，供网站显示用）
    if (homepage || twitter) {
      allScannedLinks.set(c.symbol, { homepage, twitter_screen_name: twitter });
    }
...
async function main() {
  ...
  for (const r of oldResults) {
    mergedMap.set(r.symbol, r);
  }
  for (const [sym, r] of newMap) {
    mergedMap.set(sym, r);
  }
  ...
  const output = {
    ...
    source: 'Gate latest coins by buy_start (accumulated)',
  ...
  // === 同时更新 coin_enrichment.json（供网站 API 使用）===
  // 读取旧的 enrichment 数据
  let oldEnrich = {};
  try {
    oldEnrich = JSON.parse(fs.readFileSync(enrichPath, 'utf8'));
  } catch {}
  
  // 合并所有扫描到的币种链接信息（不只是有质押的）
  const enrichment = { ...oldEnrich };
  
  // 写入所有查到链接的币种
  for (const [sym, links] of allScannedLinks) {
    const coin = filtered.find(c => c.symbol === sym) || {};
    const existing = enrichment[sym] || {};
    enrichment[sym] = {
      ...existing,
      symbol: sym,
      name: coin.name || existing.name,
      listed_at: coin.listed_at || existing.listed_at,
      markets: coin.markets || existing.markets,
      homepage: links.homepage || existing.homepage,
      twitter_screen_name: links.twitter_screen_name || existing.twitter_screen_name,
      last_scanned: new Date().toISOString(),
    };
  }
  ...
  // 写入有质押的额外信息
  for (const r of results) {
    const existing = enrichment[r.symbol] || {};
    enrichment[r.symbol] = {
      ...existing,
      staking: {
        url: r.staking.staking_url,
